...
describe('members sagas', () => {
  ...
  describe('getAllChannelMembers saga', () => {
    it('should dispatch correct action when fetching members is successful', async () => {
      ...
      expect(dispatchedActions[0]).toEqual(
        receivedChannelMembers(members as any)
      );
    });

    it('should dispatch an error when fetching members fails', async () => {
      const dispatchedActions: any[] = [];
      const mockStore = {
        getState: () => state,
        dispatch: (action: any) => dispatchedActions.push(action),
      ...
      expect(dispatchedActions[0]).toEqual(receivedChannelMembersError(error));
    });
  });
});
